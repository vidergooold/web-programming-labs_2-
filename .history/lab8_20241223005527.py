...
@login_required
def articles():
    user_articles = db.session.query(
        Lab8Articles.id,
        Lab8Articles.title,
        Lab8Articles.article_text,
        Lab8Users.login.label('author')
    ).join(Lab8Users, Lab8Articles.login_id == Lab8Users.id).filter(
        Lab8Articles.login_id == current_user.id
    ).all()
...
def create_article():
    if request.method == 'POST':
        ...
        is_public = 'is_public' in request.form  # Получаем значение checkbox

        # Создание новой статьи
        new_article = Lab8Articles(
...
def search_articles():
    if request.method == 'POST':
        ...
        if current_user.is_authenticated:
            # Авторизован: ищем в публичных и своих
            articles = Lab8Articles.query.filter(
                (Lab8Articles.is_public == True) | (Lab8Articles.login_id == current_user.id),
                Lab8Articles.title.ilike(f'%{query}%')
            ).all()
        else:
            # Неавторизован: ищем только в публичных
            articles = Lab8Articles.query.filter(
                Lab8Articles.is_public == True,
                Lab8Articles.title.ilike(f'%{query}%')
            ).all()

        return render_template('lab8/search_results.html', articles=articles, query=query)
