...
def admin_required(f):
    ...
    def decorated_function(*args, **kwargs):
        if not is_admin():
            ...
            return redirect('rgz/admin_login')
...
def add_book():
    if request.method == 'POST':
        ...
        return redirect('rgz/books_list')
...
def edit_book(book_id):
    ...
    if request.method == 'POST':
        ...
        if not title or not author or not pages or not publisher or not cover:
            flash('Все поля обязательны для заполнения!')
            return redirect('rgz/edit_book', book_id=book_id)

        try:
            pages = int(pages)
        except ValueError:
            flash('Количество страниц должно быть числом!')
            return redirect('rgz/rgz/edit_book', book_id=book_id)
