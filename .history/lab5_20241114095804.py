...
def db_connect():
    if current_app.config['DB_TYPE'] == 'postgres':
        ...
        # Подключение к SQLite с абсолютным путем к файлу
        db_path = r'C:\Users\admin\Desktop\web-programming-labs_2\database.db'
        conn = sqlite3.connect(db_path)
        conn.row_factory = sqlite3.Row
        cur = conn.cursor()
...
def db_connect():
    if current_app.config['DB_TYPE'] == 'postgres':
        ...
        conn = sqlite3.connect('C:\Users\admin\Desktop\web-programming-labs_2\database.db') 
        conn.row_factory = sqlite3.Row
        cur = conn.cursor()
    else:
        dir_path = path.dirname(path.realpath(__file__))
