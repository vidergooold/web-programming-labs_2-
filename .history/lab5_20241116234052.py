...
def db_connect():
    if current_app.config['DB_TYPE'] == 'postgres':
        conn = psycopg2.connect(
            host='127.0.0.1',
            database='ira_orlova_knowledge_base',
            user='ira_orlova_knowledge_base',
            password='123'
        )
...
def create():
    if 'login' not in session:
        return redirect(url_for('/lab5/lab5login'))
    ...
    if request.method == 'POST':
        ...
        db_close(conn, cur)
        return redirect(url_for('/lab5/lab5/lab5_home'))
...
def edit_article(article_id):
    if 'login' not in session:
        return redirect(url_for('/lab5/lab5/login'))
    ...
    if request.method == 'POST':
        ...
        db_close(conn, cur)
        return redirect(url_for('/lab5/lab5/list_articles'))
