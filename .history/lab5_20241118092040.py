...
def create():
    ...
    if current_app.config['DB_TYPE'] == 'postgres':
        cur.execute("INSERT INTO articles (users_id, title, article_text) VALUES (%s, %s, %s);", (users_id, title, article_text))
    else:
        cur.execute("INSERT INTO articles (users_id, title, article_text) VALUES (?, ?, ?);", (users_id, title, article_text))
...
def list():
    ...
    if current_app.config['DB_TYPE'] == 'postgres':
        cur.execute("SELECT * FROM articles WHERE users_id=%s ORDER BY is_favorite DESC;", (users_id,))
    else:
        cur.execute("SELECT * FROM articles WHERE users_id=? ORDER BY is_favorite DESC;", (users_id,))
...
def public_articles():
    ...
    cur.execute("SELECT articles.title, articles.article_text, users.login FROM articles JOIN users ON articles.users_id = users.id WHERE articles.is_public = TRUE;")
    articles = cur.fetchall()
