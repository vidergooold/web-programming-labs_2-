...
def settings():
    if request.method == 'POST':
        ...
        if color:
            # Устанавливаем куки с цветом и перенаправляем на эту же страницу
            resp = make_response(redirect('/lab3/settings'))
            resp.set_cookie('color', color)
            return resp

    # Получаем цвет из куки, если он есть
    color = request.cookies.get('color')
    # Рендерим страницу с текущим цветом из куки
    return make_response(render_template('lab3/settings.html', color=color))

@lab3.route('/lab3/lab3/settings')
def lab3_settings():
    return redirect('/lab3/settings')