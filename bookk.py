...
bookk = Blueprint('bookk', __name__, template_folder='templates', static_folder='static')
...
def admin_required(f):
    ...
    def decorated_function(*args, **kwargs):
        if not is_admin():
            ...
            return redirect(url_for('bookk.admin_login'))
...
@bookk.route('/add', methods=['GET', 'POST'])
@admin_required
def add_book():
    if request.method == 'POST':
        ...
        if not title or not author or not pages or not publisher or not cover:
            ...
            return redirect(url_for('bookk.add_book'))
        ...
        return redirect(url_for('bookk.books_list'))

    return render_template('bookk/add_book.html') 

@bookk.route('/edit/<int:book_id>', methods=['GET', 'POST'])
@admin_required
def edit_book(book_id):
    ...
    if request.method == 'POST':
        ...
        if not title or not author or not pages or not publisher or not cover:
            flash('Все поля обязательны для заполнения!')
            return redirect(url_for('bookk.edit_book', book_id=book_id))
    ...
    conn.close()
    return render_template('bookk/edit_book.html', book=book)

@bookk.route('/delete/<int:book_id>', methods=['POST'])
@admin_required
...
@bookk.route('/admin-login', methods=['GET', 'POST'])
def admin_login():
    if request.method == 'POST':
    ...
    return render_template('bookk/admin_login.html')

@bookk.route('/admin-logout')
def admin_logout():
    conn = get_db_connection()
